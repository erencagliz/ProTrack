...
class Profile extends Controller
{
    public function index ()
    {
        ...
        $detail = UserDetail::query()->where(['user_id' => Auth::user()->id])->first();
        $todo = Todo::query()->where(['user_id' => Auth::user()->id, 'status' => 'active'])->get();
        ...
        $targets = Target::query()->where(['user_id' => Auth::user()->id])->get();
        $offdays = Offday::query()->where(['user_id' => Auth::user()->id, 'status' => 'active'])->get();
        $files = File::query()->where(['user_id' => Auth::user()->id, 'status' => 'active'])->orderBy('created_at', 'desc')->get();
        $pauses = Pause::query()->where(['user_id' => Auth::user()->id, 'status' => 'active'])->orderBy('created_at', 'desc')->get();
        return view('pages.profile', ['files' => $files, 'pauses' => $pauses, 'offdays' => $offdays, 'targets' => $targets, 'local_permissions' => $local_permissions,'user' => $user, 'detail' => $detail, 'todo' => $todo, 'projects_active' => $projects, 'groups_active' => $groups, 'offices_active' => $offices]);
    }
    ...
    public function perm_post (Request $request)
    {
        ...
        $insert = UserDetail::query()->where(['user_id' => Auth::user()->id])->update(['custom' => $json]);
        if ($insert) {
