...
<!--begin::Head-->
<head><base href=""/>
    <title>Satış Ekle | Triooz</title><meta name="viewport" content="width=device-width, initial-scale=1.0"/>
    <meta charset="utf-8" />
                ...
                <div class="toolbar d-flex align-items-stretch">
                    ...
                    <div class="container-fluid py-6 py-lg-0 d-flex flex-column flex-lg-row align-items-lg-stretch justify-content-lg-between">
                        ...
                        <div class="page-title d-flex justify-content-center flex-column me-5">
                            <!--begin::Title-->
                            <h1 class="d-flex flex-column text-dark fw-bold fs-3 mb-0">Satış Ekle</h1>
                            <!--end::Title-->
                            <!--begin::Breadcrumb-->
                            <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 pt-1">
                                ...
                                <!--end::Item-->
                                <!--begin::Item-->
                                <li class="breadcrumb-item text-muted">Satış Ekle</li>
                                <!--end::Item-->
                            </ul>
                            <!--end::Breadcrumb-->
                        </div>
            ...
            <div class="content d-flex flex-column flex-column-fluid" id="kt_content">
                ...
                <div class="post d-flex flex-column-fluid" id="kt_post">
                    ...
                    <div id="kt_content_container" class="container-fluid">
                        <form class="card mb-6 mb-xl-9" action="{{ route('add-sale.post') }}" method="post">
                            @csrf
                            ...
                            <div class="card-body pt-5 pb-5">
                                        ...
                                        @foreach($projects as $row)
                                            <optgroup label="{{ $row->title }}">
                                                ...
                                                @foreach($users as $user)
                                                    <option data-project-id="{{ $user->project_id }}" data-office-id="{{ $user->office_id }}" value="{{ $user->id }}">{{ $user->name.' '.$user->surname }}</option>
                                                @endforeach
                                            </optgroup>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group mt-5" id="product_container">
                                ...
                                <div class="form-group mt-5" id="quantity_container">
                                ...
                                <div class="form-group mt-5" id="date_container">
                                    ...
                                    <input type="date" max="{{ date('Y-m-d') }}" value="{{ date('Y-m-d') }}" name="created_at" class="form-control"/>
                                </div>

                                <div class="form-group mt-5" id="iban_container">
                                    <label class="form-label">IBAN Onayı:</label>
                                    <select name="custom[iban_verify]" class="form-select form-select-solid" data-control="select2" >
                                ...
                                <div class="form-group mt-5" id="sms_container">
                                    <label class="form-label">SMS Onayı:</label>
                                    <select name="custom[sms_verify]" class="form-select form-select-solid" data-control="select2" >
                                        <option selected >Seçiniz...</option>
                                        <option value="true">Evet</option>
                                        <option value="false">Hayır</option>
                                    </select>
                                ...
                                <div class="form-group mt-5" id="phone_container">
            ...
            <div class="footer py-4 d-flex flex-lg-column" id="kt_footer">
                ...
                <div class="container-fluid d-flex flex-column flex-md-row align-items-center justify-content-between">
                    ...
                    <div class="text-dark order-2 order-md-1">
                        ...
                        <a class="text-gray-800 text-hover-primary">trApp</a>
                    </div>
                    <!--end::Copyright-->
                </div>
                <!--end::Container-->
...
<script>
    $(document).ready(function () {
        ...
        $('select[name=user_id]').change(function () {
            ...
            let office_id = $(this).find(':selected').data('office-id');
            $('input[name=office_id]').val(office_id);
            $('input[name=project_id]').val(project_id);
            $('select[name=product_id]').html('<option selected disabled>Yükleniyor...</option>');
            $.ajax({
                url: "{{ route('ajax.return_products_with_option') }}/" + project_id,
                type: "GET",
                success: function (response) {
                    if (response) {
                        ...
                        $('select[name=product_id]').html('<option selected disabled>Seçiniz...</option>'+response);
                    } else {
                        $('#product_container').hide();
                        $('#quantity_container').hide();
                        $('#date_container').hide();
                        ...
                        $('select[name=product_id]').html('<option selected disabled>Seçiniz...</option>');
                        toastr.error("Kullanıcının projesine ait hiçbir ürün bulunamadı.");
                    }
                }
            });
        });
        $('select[name=product_id]').change(function () {
            let point = $(this).find(':selected').data('point');
            $('input[name=point]').val(point);
        });
        $('select[name=quantity]').change(function () {
            let points = $('select[name=product_id]').find(':selected').data('point');
            let quantity = $(this).find(':selected').val();
            let total_point = points * quantity;
            $('input[name=point]').val(total_point);
        });
    });
</script>
