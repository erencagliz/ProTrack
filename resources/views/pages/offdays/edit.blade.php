...
<!--begin::Head-->
<head><base href=""/>
    <title>İzin Düzenle | Triooz</title><meta name="viewport" content="width=device-width, initial-scale=1.0"/>
    <meta charset="utf-8" />
                ...
                <div class="toolbar d-flex align-items-stretch">
                    ...
                    <div class="container-fluid py-6 py-lg-0 d-flex flex-column flex-lg-row align-items-lg-stretch justify-content-lg-between">
                        ...
                        <div class="page-title d-flex justify-content-center flex-column me-5">
                            <!--begin::Title-->
                            <h1 class="d-flex flex-column text-dark fw-bold fs-3 mb-0">İzin Düzenle</h1>
                            <!--end::Title-->
                            <!--begin::Breadcrumb-->
                            <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 pt-1">
                                ...
                                <!--end::Item-->
                                <!--begin::Item-->
                                <li class="breadcrumb-item text-muted">İzin Düzenle</li>
                                <!--end::Item-->
                            </ul>
                            <!--end::Breadcrumb-->
                        </div>
            ...
            <div class="content d-flex flex-column flex-column-fluid" id="kt_content">
                ...
                <div class="post d-flex flex-column-fluid" id="kt_post">
                    ...
                    <div id="kt_content_container" class="container-fluid">
                        <form class="card mb-6 mb-xl-9" action="{{ route('offdays.edit.post') }}" method="post">
                            @csrf
                            <input type="hidden" name="id" value="{{ $offday->id }}"/>
                            ...
                            <div class="card-body pt-5 pb-5">
                                        ...
                                        @foreach($projects as $row)
                                            <optgroup label="{{ $row->title }}">
                                                @php
                                                    ...
                                                    ->select(['users.*', 'user_details.project_id as project_id', 'user_details.office_id as office_id'])
                                                    ->join('user_details', 'users.id', '=', 'user_details.user_id')
                                                    ->where(['users.status' => 'active', 'user_details.project_id' => $row->id])
                                                    ->get();
                                                @endphp
                                                @foreach($users as $user)
                                                    <option {{ $offday->user_id == $user->id ? "selected" : "" }} value="{{ $user->id }}">{{ $user->name.' '.$user->surname }}</option>
                                                @endforeach
                                            </optgroup>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group mt-5" id="date_container">
                                    <label class="form-label">Tarih:</label>
                                    <input type="date" name="date" class="form-control" value="{{ $offday->date }}"/>
                                </div>

                                <div class="form-group mt-5" id="type_container">
                                    <label class="form-label">Tip:</label>
                                    <select name="type" class="form-select form-select-solid" data-control="select2" >
                                        <option disabled>Seçiniz...</option>
                                        <option {{ $offday->type == "date" ? "selected" : "" }} value="date">Gün</option>
                                        <option {{ $offday->type == "datetime" ? "selected" : "" }} value="datetime">Süre</option>
                                    </select>
                                </div>

                                <div id="type_date_container">
                                    <div class="form-group mt-5">
                                        <label class="form-label">İzin Tipi:</label>
                                        <select name="date_type" class="form-select form-select-solid" data-control="select2" >
                                            <option disabled>Seçiniz...</option>
                                            <option {{ $offday->date_type == "yearly" ? "selected" : "" }} value="yearly">Yıllık</option>
                                            <option {{ $offday->date_type == "birth" ? "selected" : "" }} value="birth">Doğum</option>
                                            <option {{ $offday->date_type == "free" ? "selected" : "" }} value="free">Ücretsiz</option>
                                            <option {{ $offday->date_type == "marriage" ? "selected" : "" }} value="marriage">Evlilik</option>
                                            <option {{ $offday->date_type == "death" ? "selected" : "" }} value="death">Ölüm</option>
                                        </select>
                                    </div>
                                </div>

                                <div id="type_datetime_container">
                                    <div class="form-group mt-5">
                                        <label class="form-label">Toplam Eksik Süre:</label>
                                        <input value="{{ $offday->time }}" type="time" name="total_time" class="form-control"/>
                                    </div>
                                    ...
                                    <div id="kt_docs_repeater_basic">
                                        ...
                                        <div class="form-group">
                                            <div data-repeater-list="kt_docs_repeater_basic">
                                                @if($offday->type == "datetime")
                                                    @foreach(json_decode($offday->movements, true) as $key => $value)
                                                        <div class="mt-5" data-repeater-item>
                                                            <div class="form-group row">
                                                                <div class="col-md-4">
                                                                    <label class="form-label">Durum:</label>
                                                                    <select class="form-control" name="move">
                                                                        <option value="">Seçiniz...</option>
                                                                        <option {{ $value['move'] == "in" ? "selected" : "" }} value="in">Girdi</option>
                                                                        <option {{ $value['move'] == "out" ? "selected" : "" }} value="out">Çıktı</option>
                                                                    </select>
                                                                </div>
                                                                <div class="col-md-4">
                                                                    <label class="form-label">Saat:</label>
                                                                    <input value="{{ $value['time'] }}" type="time" name="time" class="form-control mb-2 mb-md-0" placeholder="Saat" />
                                                                </div>
                                                    ...
                                                    <div class="mt-5" data-repeater-item>
                                                        <div class="form-group row">
                                                            <div class="col-md-4">
                                                                ...
                                                                <select class="form-control" name="move">
                                                                    <option value="">Seçiniz...</option>
                                                                    <option value="in">Girdi</option>
                                                                    <option value="out">Çıktı</option>
                                                                </select>
                                                            </div>
                                                            <div class="col-md-4">
                                                                <label class="form-label">Saat:</label>
                                                                <input type="time" name="time" class="form-control mb-2 mb-md-0" placeholder="Saat" />
                                                            </div>
                                                            <div class="col-md-4 text-end">
                                                                <a href="javascript:;" data-repeater-delete class="btn btn-sm btn-light-danger mt-3 mt-md-8">
                                                                    <i class="la la-trash-o"></i>Sil
...
<script>
    ...
    $(document).ready(function () {
        @if($offday->type == "date")
            $('#type_date_container').show();
            $('#type_datetime_container').hide();
        @else
            $('#type_date_container').hide();
            $('#type_datetime_container').show();
        @endif
